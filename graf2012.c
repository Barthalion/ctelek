//program p3 KTB Oct 11, 1996; KTZ 2013
...
do
    {
      ...
      case 1: //Gnp
        ...
        case 3: //Gnk, transEA
          ...
          */
  /* case 9: // rysowanie Gnp w GraphViz
              {
                // pdrukAGrViz(plik,n,A);
                break;
              } // 9
          */
          default:
...
{ int i,j,x,y;
  ...
  for (i=1;i<=n-1;i++)
  for (j=i+1;j<=n;j++)
    if (A[i][j]) fprintf(out,"1"); else fprintf(out,"0");
  fprintf(out," ");
  dpn=2*M_PI/n;
  for (i=1;i<=n;i++)
  {
